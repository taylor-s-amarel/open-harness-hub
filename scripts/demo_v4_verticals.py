#!/usr/bin/env python3
"""Demo: Real-estate DD + M&A DD + Aviation Safety + Food Safety (verticals 14/15/16/17)."""
...
def main() -> int:
    run_block("Real Estate DD",     "pipeline/real-estate-dd-review",     "data/realestate-samples", "dd_packet")
    run_block("M&A DD",              "pipeline/ma-dd-review",              "data/ma-samples",         "target_dd_packet")
    run_block("Aviation Safety",     "pipeline/aviation-safety-review",    "data/aviation-samples",   "incident_report")
    run_block("Food Safety (FSMA)",  "pipeline/food-safety-review",        "data/foodsafety-samples", "facility_packet")
    return 0
